...
import {
  ...
  addPurchaseBatch, addUsage, stocktakeAdjust,
  listPurchases, listUsages, listStocktakes,
  voidRecord, permanentlyDelete,
  computeStock, computeAvgCost,
} from "./inventory.js";

const TYPE_LABELS = { packaging: "包材", resale: "現貨商品" };
...
export async function renderInventoryPage(container) {
  ...
  function renderList() {
    ...
    listEl.innerHTML = filtered.map((item) => {
      ...
      const avgCost = computeAvgCost(item);
        ...
        <div class="card" style="margin-bottom:10px;${isArchived ? "opacity:0.55;" : ""}" data-item-row="${item.id}">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">
            ...
            <div style="text-align:right;">
              <div style="font-family:var(--font-mono);font-size:18px;font-weight:700;color:${isLow ? "var(--rose)" : "var(--ink)"};">${stock}</div>
          ...
          <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;">
            <button class="btn btn-secondary" data-detail="${item.id}" style="padding:7px 14px;font-size:13px;">記錄</button>
  ...
  function openModal(innerHtml) {
    ...
    overlay.innerHTML = `<div class="card" style="max-width:520px;width:100%;max-height:85vh;overflow-y:auto;" id="modal-box">${innerHtml}</div>`;
  ...
  function openItemModal(item = null) {
    ...
    const overlay = openModal(`
      ...
      <div class="field"><label>低庫存提醒門檻（選填）</label><input type="number" id="m-threshold" value="${item?.lowStockThreshold || ""}" /></div>
    ...
    overlay.querySelector("#m-save").addEventListener("click", async () => {
      ...
      try {
        if (isEdit) {
          await updateItem(item.id, { name, category, lowStockThreshold: threshold });
        } else {
          const type = overlay.querySelector("#m-type").value;
          await createItem({ name, type, category, lowStockThreshold: threshold });
        }
  ...
  function openPurchaseModal() {
    ...
    function itemOptions(selected) {
      return `<option value="">選擇項目</option>` + items.filter(i=>i.status!=='archived').map((i) =>
  ...
  function openStocktakeModal() {
    const overlay = openModal(`
      ...
      <div class="field"><label>項目</label>
        <select id="s-item">${items.filter(i=>i.status!=='archived').map((i) => `<option value="${i.id}">${i.name}（系統目前：${computeStock(i)}）</option>`).join("")}</select>
      </div>
  ...
  async function openDetailModal(itemId) {
    ...
    overlay.querySelector("#modal-box").innerHTML = `
      ...
      <div class="hint" style="margin-bottom:14px;">目前庫存 ${computeStock(item)} · 均價 $${computeAvgCost(item).toFixed(2)}</div>
