...
// （管理員以上）更換品牌圖案
// ============================================================
import { currentSession, ROLE_LABELS, updateMyNickname, markHomeScreenTipSeen } from "./auth.js?v=20260830-95";
import { showToast, friendlyErrorMessage } from "./utils.js?v=20260830-95";
import { openModal } from "./modal-ui.js?v=20260830-95";
import { uploadImageToCloudinary, saveBrandLogoUrl } from "./settings.js?v=20260830-95";
...
/**
 * 獨立的「加到主畫面」教學彈窗，跟個人資料/暱稱設定完全分開——
 * 兩件事塞在同一個彈窗裡，對已經設定過暱稱的人來說會顯得很突兀
 * （明明暱稱都設定好了，怎麼又跳出來要求做什麼）。
 * 不管是按「知道了」還是直接關掉，都算看過，之後不會再自動跳出來。
 */
export function openHomeScreenTipModal() {
  const overlay = openModal(`
    <h3 style="margin-bottom:16px;">📱 可以把系統加到手機主畫面</h3>
    <div style="font-size:14px;color:var(--text-primary);line-height:1.8;margin-bottom:20px;">
    ...
    <div style="display:flex;justify-content:flex-end;">
      <button class="btn btn-primary" id="hst-ok">知道了</button>
    </div>
  `, 420);

  overlay.querySelector("#modal-close-x")?.addEventListener("click", () => markHomeScreenTipSeen());
  overlay.querySelector("#hst-ok").addEventListener("click", () => {
