...
// 訂單管理頁面 UI
// ============================================================
import { showToast, linkifyErrorMessage, friendlyErrorMessage } from "./utils.js?v=20260830-99";
import { currentSession, wireNameResolution } from "./auth.js?v=20260830-99";
import {
...
} from "./orders.js?v=20260830-99";
import { listItems, buildItemsIndex, ORDERABLE_TYPES } from "./items.js?v=20260830-99";
import { listContacts, createContact, ORDER_CHANNELS } from "./contacts.js?v=20260830-99";
import { printOrderSlip, printShippingList } from "./print-slip.js?v=20260830-99";
import { exportOrders } from "./export-xlsx.js?v=20260830-99";
import { setFab, clearFab } from "./fab-ui.js?v=20260830-99";
import { openSearchPicker } from "./picker-ui.js?v=20260830-99";
import { openModal, openCustomTextModal, confirmDialog } from "./modal-ui.js?v=20260830-99";
...
/**
 * 作廢訂單的確認狀態徽章。舊資料（這個功能上線之前就已經作廢的訂單）
 * 沒有這個欄位，一律當作「已確認」處理，不會被誤判成待確認、
 * 也不會完全不顯示狀態讓人搞不清楚。
 * （相容判斷：欄位名稱曾經改過一次，舊的 needsVoidReview:true 也要
 * 認得，不然剛好卡在改名字那個時間點作廢的訂單會找不到。）
 */
function getVoidReviewStatus(order) {
  if (order.voidReviewStatus) return order.voidReviewStatus;
  if (order.needsVoidReview) return "pending";
  return "confirmed";
}
function voidReviewBadgeHtml(order) {
  const status = getVoidReviewStatus(order);
...
export async function renderOrdersPage(container, initialFilter = null) {
  ...
  function getFilteredOrders() {
    ...
    if (filterQuick === "today") {
      ...
      filtered = filtered.filter((o) => o.voided && getVoidReviewStatus(o) === "pending");
    }
  ...
  function renderOrderDetailPage(orderId) {
    ...
    container.innerHTML = `
      ...
      ${(order.voided && getVoidReviewStatus(order) === "pending" && currentSession.member?.role === "superadmin") ? `
      ...
      <div class="card" style="margin-bottom:16px;">
        ...
        ${order.voided && order.voidedByName ? `<div class="hint" style="margin-top:6px;">作廢紀錄：<span data-resolve-email="${order.voidedBy || ""}">${order.voidedByName}</span></div>` : ""}
