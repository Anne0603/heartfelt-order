...
export function setFab(actions) {
  ...
  if (actions.length === 1) {
    fabContainer.innerHTML = `<button class="fab-main" aria-label="${actions[0].label}" title="${actions[0].label}">${actions[0].icon}</button>`;
    ...
    fabContainer.innerHTML = `
      ${actions.map((a) => `
        <div class="fab-sub-row ${open ? "show" : ""}">
          ...
          <button class="fab-sub-btn" data-idx aria-label="${a.label}">${a.icon}</button>
        </div>
      `).join("")}
      <button class="fab-main ${open ? "open" : ""}" aria-label="新增">${open ? "✕" : "＋"}</button>
    `;
